import multiprocessing as mp # Use mp prefix for clarity
from typing import Dict, Optional, Listimport time # For join timeout

from .tui_models import TUIMonitorData, MonitorTUIStatus# Import TickerMonitor carefully
try:
    ...
    TickerMonitor = None # This will cause issues if not resolved

class TUIMonitorManager:
    def __init__(self):
        self._monitors_data: Dict[str, TUIMonitorData] = {}
        self._processes: Dict[str, mp.Process] = {}
        self._queues: Dict[str, mp.Queue] = {} # Trade order queues
    ...
    def get_all_monitor_data(self) -> List[TUIMonitorData]:
        # Ensure status is up-to-date based on process existence/liveness
        for tui_id, data in self._monitors_data.items():
            ...
            if process:
                if process.is_alive():
                    if data.status not in [MonitorTUIStatus.RUNNING, MonitorTUIStatus.STARTING, MonitorTUIStatus.STOPPING]:
                        data.status = MonitorTUIStatus.RUNNING # Correct if found alive unexpectedly
                else: # Process exists but not alive
                    if data.status == MonitorTUIStatus.RUNNING or data.status == MonitorTUIStatus.STOPPING:
                        data.status = MonitorTUIStatus.STOPPED
                        data.last_error_message = f"Process ended unexpectedly (exitcode {process.exitcode})."
                    # If it was STARTING and died, mark as ERROR or STOPPED
            ...
            elif data.status == MonitorTUIStatus.RUNNING: # No process but status is RUNNING
                data.status = MonitorTUIStatus.UNKNOWN # Status desync
    ...
    def start_monitor(self, tui_id: str) -> bool:
        if TickerMonitor is None:
            ...
            if monitor_data:                monitor_data.status = MonitorTUIStatus.ERROR
                monitor_data.last_error_message = "TickerMonitor class not loaded."
            return False

        monitor_data = self.get_monitor_data(tui_id)
        if not monitor_data or tui_id in self._processes:import multiprocessing as mp # Use mp prefix for clarity
from typing import Dict, Optional, Listimport time # For join timeout
...
class TUIMonitorManager:
    ...
    def add_monitor_config(self, config: TUIMonitorData) -> str:
        if config.tui_id in self._monitors_data:
            raise ValueError(f"Monitor with TUI ID {config.tui_id} already exists.")
        self._monitors_data[config.tui_id] = config        return config.tui_id
    ...
    def start_monitor(self, tui_id: str) -> bool:
        ...
        if tui_id in self._processes and self._processes[tui_id].is_alive():
            ...
            monitor_data.status = MonitorTUIStatus.RUNNING # Ensure status is correct
            return True # Or False, depending on desired behavior for "already running"

        monitor_data.status = MonitorTUIStatus.STARTING
        monitor_data.last_error_message = None # Clear previous errors

        try:
            # Create a dedicated queue for this monitor instance
            trade_queue = mp.Queue()
            self._queues[tui_id] = trade_queue
            monitor_data.trade_order_queue = trade_queue # Store for TUI access later

            # Target function for the process
            def _run_monitor_process(config_dict, q):
                # This function runs in a separate process
                # Re-import TickerMonitor here if necessary, or ensure it's picklable
                # For simplicity, assuming TickerMonitor can be instantiated directly
                # and its dependencies are available in the new process environment.
                try:
                    from stock_monitoring_app.monitoring.ticker_monitor import TickerMonitor as ActualTickerMonitor
                    # Create and run the TickerMonitor instance                    monitor_instance = ActualTickerMonitor(
                        ticker=config_dict['ticker'],
                        monitor_interval_seconds=config_dict['monitor_interval_seconds'],
                        trade_order_queue=q, # Pass the multiprocessing queue
                        entry_price=config_dict['entry_price'],
                        process_name=config_dict['process_name'],
                        backtest_scope=config_dict['backtest_scope']
                    )
                    monitor_instance.run() # This is a blocking call
                except Exception as e:
                    print(f"ERROR in TickerMonitor process {config_dict.get('process_name', 'Unknown')}: {e}")                    # Optionally, put an error message onto the queue or handle logging
                finally:
                    print(f"INFO: TickerMonitor process {config_dict.get('process_name', 'Unknown')} finished.")


            # Prepare a dictionary of basic types for the process target
            config_for_process = {
                ...
                "process_name": monitor_data.process_name, # Use the generated name
            ...
            process = mp.Process(
                ...
                daemon=True # So it exits when the main TUI process exits
            )
            process.start()
            self._processes[tui_id] = process
            monitor_data.process = process # Store process handle in TUIMonitorData
            monitor_data.status = MonitorTUIStatus.RUNNING # Assuming it starts okay, queue worker will verify
            ...
            monitor_data.last_error_message = str(e)            if tui_id in self._queues: del self._queues[tui_id] # Clean up queue
            if tui_id in self._processes: del self._processes[tui_id] # Clean up process ref
            return False
    ...
    def stop_monitor(self, tui_id: str, timeout_seconds: int = 5) -> bool:
        ...
        if not process or not process.is_alive():
            ...
            if tui_id in self._processes: del self._processes[tui_id] # Clean up ref
            if tui_id in self._queues and self._queues[tui_id] is not None:
                try:
                    self._queues[tui_id].close() # Close the queue
        ...
        try:
            # The TickerMonitor's run loop checks `self._running`. We need a way to set that.
            # For multiprocessing, the simplest robust way is often termination.
            # If TickerMonitor had an IPC mechanism to receive a "stop" command (e.g., via its queue),
            # that would be cleaner. For now, we'll use terminate then join.
            # TickerMonitor's `stop()` method sets an internal flag. If it's running in a separate
            # process, calling `monitor_instance.stop()` from the TUI process has no effect.            process.terminate() # Send SIGTERM
            process.join(timeout=timeout_seconds) # Wait for it to exit

            if process.is_alive():
                print(f"WARN: Process for {monitor_data.ticker} did not terminate gracefully. Forcing kill...")
                process.kill() # Send SIGKILL
                process.join(timeout=1) # Wait a bit more

            exit_code = process.exitcode            print(f"INFO: Process for {monitor_data.ticker} stopped. Exit code: {exit_code}")
            monitor_data.status = MonitorTUIStatus.STOPPED
            ...
            # Don't return False immediately, still try to clean up
        finally:
            # Clean up
            if tui_id in self._processes: del self._processes[tui_id]
            if tui_id in self._queues and self._queues[tui_id] is not None:
                try:
                    self._queues[tui_id].close()
                    # Drain the queue to allow the feeder thread to join
    ...
    def delete_monitor_config(self, tui_id: str) -> bool:
        ...
        if monitor_data.status == MonitorTUIStatus.RUNNING or \
           monitor_data.status == MonitorTUIStatus.STARTING or \
           monitor_data.status == MonitorTUIStatus.STOPPING :
            print(f"INFO: Monitor {monitor_data.ticker} ({tui_id[:8]}) is active. Stopping before delete.")
            self.stop_monitor(tui_id) # Ensure it's stopped
        ...
        if tui_id in self._monitors_data:
            ...
            # Process and queue should have been cleaned by stop_monitor
            if tui_id in self._processes: del self._processes[tui_id] # Defensive
            if tui_id in self._queues: del self._queues[tui_id] # Defensive
            return True
        return False

    def get_trade_queue_for_monitor(self, tui_id: str) -> Optional[mp.Queue]:
        """Returns the trade order queue for a given monitor TUI ID."""
        return self._queues.get(tui_id)

    def cleanup_all_monitors(self):
        """Stops all running monitors. Typically called on TUI exit."""
        print("INFO: Cleaning up all active monitors...")
        # Iterate over a copy of keys as self._monitors_data might change during stop
        for tui_id in list(self._monitors_data.keys()):
            monitor_data = self._monitors_data.get(tui_id)
            if monitor_data and (monitor_data.status == MonitorTUIStatus.RUNNING or \
                                 monitor_data.status == MonitorTUIStatus.STOPPING or \
                                 monitor_data.status == MonitorTUIStatus.STARTING) :
                print(f"      Stopping {monitor_data.ticker} ({tui_id[:8]})...")
                self.stop_monitor(tui_id, timeout_seconds=2) # Shorter timeout on mass cleanup

...
def find_best_result_file(crypto_id, param_set=None):
    ...
    Priority:
    1. New format: backtest_results/best_params_{crypto}_{strategy}_bayesian.json
    2. Old format: backtest_results/best_params_{crypto}_{timeframe}_{interval}_{param_set}.json
    """
    ...
    new_pattern = os.path.join(results_dir, f"best_params_{crypto_id}_*_bayesian.json")
    ...
    if param_set:
        ...
        old_format = os.path.join(results_dir, f"best_params_{crypto_id}_{DEFAULT_TIMEFRAME}_{DEFAULT_INTERVAL}_{param_set}.json")
